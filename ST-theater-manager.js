...
(function() {
  ...
  let theaterData = {
    ...
    // ★ v4.3 更新：新增排序设置
    settings: {
      favoriteOnTop: true,           // 旧设置，保留兼容
      favoritesPageSort: true,       // 收藏页：开启=置顶+频率排序，关闭=仅用户顺序
      allPageSort: false             // 全部页：开启=频率排序，关闭=仅用户顺序（默认关闭）
    }
  };
    ...
    try {
      ...
      if (context) {
        ...
        if (saved) {
          ...
          // ★ 兼容旧版本设置
          if (!theaterData.settings) {
            theaterData.settings = { favoriteOnTop: true, favoritesPageSort: true, allPageSort: false };
          } else {
            if (theaterData.settings.favoritesPageSort === undefined) theaterData.settings.favoritesPageSort = true;
            if (theaterData.settings.allPageSort === undefined) theaterData.settings.allPageSort = false;
          }
        }
      }
    } catch (e) {}
  ...
  function getFilteredPrompts(forGenerate = false) {
    ...
    prompts.sort((a, b) => {
      ...
      if (currentTab === 'favorites' && !forGenerate) {
        if (theaterData.settings.favoritesPageSort) {
          // 开启：收藏置顶 + 使用频率排序
      ...
      if (currentTab === 'all' && !forGenerate) {
        if (theaterData.settings.allPageSort) {
          // 开启：按使用频率排序
          return (b.useCount || 0) - (a.useCount || 0);
        } else {
          // 关闭（默认）：仅按用户拖动顺序
  ...
  function getFolderPrompts(folderId) {
    return theaterData.prompts.filter(p => p.folderId === folderId).sort((a, b) => {
      if (a.sortOrder !== undefined && b.sortOrder !== undefined) return a.sortOrder - b.sortOrder;
      return (b.useCount || 0) - (a.useCount || 0);
    });
  }
  ...
  function showUndoBar(item) {
    const $ = getJQuery();
    hideUndoBar();
    const undoBar = $('#tm-undo-bar');
    undoBar.html(`<span class="tm-undo-text">已删除「${escapeHtml(item.title || item.name)}」</span><button class="tm-undo-action-btn">撤销</button><div class="tm-undo-timer">10</div>`).addClass('show');
    let countdown = 10;
    const timerEl = undoBar.find('.tm-undo-timer');
    undoTimeout = setInterval(() => {
      countdown--;
      timerEl.text(countdown);
      if (countdown <= 0) { hideUndoBar(); deletedItem = null; }
    }, 1000);
  }

  function hideUndoBar() {
    const $ = getJQuery();
    if (undoTimeout) { clearInterval(undoTimeout); undoTimeout = null; }
    $('#tm-undo-bar').removeClass('show').html('');
  }

  async function undoDelete() {
    if (!deletedItem) return;
    if (deletedItem.type === 'folder') { theaterData.folders.push(deletedItem.data); }
    else { theaterData.prompts.push(deletedItem.data); }
    await saveData();
    hideUndoBar();
    deletedItem = null;
    TheaterManager.updateCategoryFilter();
    TheaterManager.renderPromptList();
  }
  ...
  function findDuplicates(entries) {
    ...
    entries.forEach(entry => {
      const existing = theaterData.prompts.find(p => p.content.trim() === entry.content.trim());
      if (existing) duplicates.push({ entry, existing });
  ...
  function renderCategoryTags(categories) {
    ...
    return cats.map(c => `<span class="card-category">${escapeHtml(c)}</span>`).join('');
  }
  ...
  function initDragAndDrop() {
    ...
    doc.off('.tmDrag');
    doc.on('touchstart.tmDrag mousedown.tmDrag', '.tm-drag-handle', function(e) {
      e.preventDefault();
      const card = $(this).closest('.prompt-card, .folder-card');
      draggedItem = card.data('id');
      draggedType = card.hasClass('folder-card') ? 'folder' : 'prompt';
      card.addClass('dragging');
    });
    doc.on('touchmove.tmDrag mousemove.tmDrag', function(e) {
      if (!draggedItem) return;
      const touch = e.type === 'touchmove' ? e.originalEvent.touches[0] : e;
      const target = getParentWindow().document.elementFromPoint(touch.clientX, touch.clientY);
      const targetCard = $(target).closest('.prompt-card, .folder-card, .folder-drop-zone');
      $('.drop-target').removeClass('drop-target');
      if (targetCard.length && targetCard.data('id') !== draggedItem) {
        targetCard.addClass('drop-target');
      }
    });
    doc.on('touchend.tmDrag mouseup.tmDrag', async function(e) {
      if (!draggedItem) return;
      ...
      if (targetCard.length && targetCard.data('id') !== draggedItem) {
        ...
        if (draggedType === 'prompt') {
          if (targetCard.hasClass('folder-card') || targetCard.hasClass('folder-drop-zone')) {
            const prompt = theaterData.prompts.find(p => p.id === draggedItem);
            if (prompt) { prompt.folderId = targetId; await saveData(); }
          } else {
            await reorderPrompts(draggedItem, targetId);
          }
        } else if (draggedType === 'folder') {
          await reorderFolders(draggedItem, targetId);
        }
      }
  ...
  async function reorderPrompts(draggedId, targetId) {
    const draggedPrompt = theaterData.prompts.find(p => p.id === draggedId);
    const targetPrompt = theaterData.prompts.find(p => p.id === targetId);
    if (!draggedPrompt || !targetPrompt) return;
    const prompts = theaterData.prompts.filter(p => p.folderId === draggedPrompt.folderId);
    prompts.forEach((p, i) => p.sortOrder = i);
  ...
  async function reorderFolders(draggedId, targetId) {
    const draggedIndex = theaterData.folders.findIndex(f => f.id === draggedId);
    const targetIndex = theaterData.folders.findIndex(f => f.id === targetId);
    if (draggedIndex === -1 || targetIndex === -1) return;
    const [folder] = theaterData.folders.splice(draggedIndex, 1);
    theaterData.folders.splice(targetIndex, 0, folder);
    theaterData.folders.forEach((f, i) => f.sortOrder = i);
    await saveData();
  }
  const TheaterManager = {
    ...
    switchTab(tab) {
      ...
      batchSelectMode = false;
      selectedIds.clear();
      $('#theater-manager-modal .tm-tab').removeClass('active');
      $(`#theater-manager-modal .tm-tab[data-tab="${tab}"]`).addClass('active');
    ...
    renderPromptList() {
      ...
      if (currentTab === 'all') {
        folders.forEach(folder => {
          ...
          if (searchText && !folderNameMatch && matchingPromptsInFolder.length === 0) return;
          if (categoryFilter && matchingPromptsInFolder.length === 0) return;
          ...
          const countDisplay = (searchText || categoryFilter) ? `${matchingPromptsInFolder.length}/${folderPrompts.length}` : `${folderPrompts.length}`;
          html += `<div class="folder-card ${isExpanded ? 'expanded' : ''}" data-id="${folder.id}"><div class="folder-header" data-folder-id="${folder.id}"><span class="tm-drag-handle">☰</span><span class="folder-icon">${isExpanded ? '📂' : '📁'}</span><span class="folder-name">${escapeHtml(folder.name)}</span><span class="folder-count">${countDisplay}</span><div class="folder-actions"><button class="card-btn" data-action="editFolder" data-id="${folder.id}">✏️</button><button class="card-btn card-btn-danger" data-action="deleteFolder" data-id="${folder.id}">🗑️</button></div></div>${isExpanded ? `<div class="folder-content">${displayPrompts.map(p => this.renderPromptCard(p)).join('')}</div>` : ''}</div>`;
        });
      }
    ...
    renderPromptCard(p) {
      const folderInfo = p.folderId ? theaterData.folders.find(f => f.id === p.folderId) : null;
      const folderBadge = folderInfo && currentTab !== 'all' ? `<span class="card-folder-badge">📁 ${escapeHtml(folderInfo.name)}</span>` : '';
      return `<div class="prompt-card ${p.favorite ? 'favorite' : ''} ${selectedIds.has(p.id) ? 'selected' : ''}" data-id="${p.id}"><span class="tm-drag-handle">☰</span>${batchSelectMode ? `<input type="checkbox" class="tm-batch-check" data-id="${p.id}" ${selectedIds.has(p.id) ? 'checked' : ''}>` : ''}<div class="card-body"><div class="card-row-title"><span class="card-title">${p.favorite ? '⭐ ' : ''}${escapeHtml(p.title)}</span><div class="card-categories">${renderCategoryTags(p.category)}</div>${folderBadge}</div><div class="card-row-preview">${escapeHtml(p.content?.substring(0, 100) || '')}${(p.content?.length || 0) > 100 ? '...' : ''}</div><div class="card-row-footer"><span class="card-stats">使用${p.useCount || 0}次 ${p.lastUsed ? `· ${formatDate(p.lastUsed)}` : ''}</span><div class="card-btns"><button class="card-btn" data-action="toggleFavorite" data-id="${p.id}">${p.favorite ? '⭐' : '☆'}</button><button class="card-btn" data-action="moveToFolder" data-id="${p.id}">📁</button><button class="card-btn" data-action="showPreview" data-id="${p.id}">👁️</button><button class="card-btn" data-action="insertToInput" data-id="${p.id}">📋</button><button class="card-btn" data-action="openEditModal" data-id="${p.id}">✏️</button><button class="card-btn card-btn-danger" data-action="deletePrompt" data-id="${p.id}">🗑️</button></div></div></div></div>`;
    },

    openFolderModal(editId = null) {
      const $ = getJQuery();
      $('#tm-folderModal').remove();
      const folder = editId ? theaterData.folders.find(f => f.id === editId) : null;
      $('body').append(`<div id="tm-folderModal" class="tm-modal-overlay"><div class="tm-modal" style="max-width:400px;"><div class="tm-modal-header"><h2>${folder ? '✏️ 编辑文件夹' : '📁 新建文件夹'}</h2><button class="tm-modal-close tm-folder-action" data-action="closeFolderModal">×</button></div><div class="tm-modal-body"><div class="form-group"><label>文件夹名称</label><input type="text" id="tm-folderName" class="form-input" value="${folder ? escapeHtml(folder.name) : ''}" placeholder="输入文件夹名称..."></div></div><div class="tm-modal-footer"><button class="tm-btn tm-btn-secondary tm-folder-action" data-action="closeFolderModal">取消</button><button class="tm-btn tm-btn-primary tm-folder-action" data-action="saveFolder" data-id="${editId || ''}">保存</button></div></div></div>`);
      $('#tm-folderName').focus();
    },

    closeFolderModal() { getJQuery()('#tm-folderModal').remove(); },

    async saveFolder(editId) {
      const $ = getJQuery();
      const name = $('#tm-folderName').val().trim();
      if (!name) { alert('请输入文件夹名称'); return; }
      if (editId) {
        const folder = theaterData.folders.find(f => f.id === editId);
        if (folder) folder.name = name;
      } else {
        theaterData.folders.push({ id: generateId(), name: name, sortOrder: theaterData.folders.length, createdAt: Date.now() });
      }
      await saveData();
    ...
    async deleteFolder(id) {
      const folder = theaterData.folders.find(f => f.id === id);
      if (!folder) return;
      const folderPrompts = theaterData.prompts.filter(p => p.folderId === id);
      if (folderPrompts.length > 0) {
        if (!confirm(`文件夹内有 ${folderPrompts.length} 个小剧场，删除后将移出文件夹。确定删除？`)) return;
        folderPrompts.forEach(p => delete p.folderId);
      }
      deletedItem = { type: 'folder', data: { ...folder } };
      theaterData.folders = theaterData.folders.filter(f => f.id !== id);
      await saveData();
      this.renderPromptList();
      showUndoBar({ name: folder.name });
    },
    ...
    moveToFolder(promptId) {
      const $ = getJQuery();
      const prompt = theaterData.prompts.find(p => p.id === promptId);
      if (!prompt) return;
      $('#tm-moveToFolderModal').remove();
      const folders = theaterData.folders || [];
      $('body').append(`<div id="tm-moveToFolderModal" class="tm-modal-overlay"><div class="tm-modal" style="max-width:400px;"><div class="tm-modal-header"><h2>📁 移动到文件夹</h2><button class="tm-modal-close tm-move-action" data-action="closeMoveModal">×</button></div><div class="tm-modal-body"><p style="margin-bottom:12px;color:var(--tm-text-muted);">「${escapeHtml(prompt.title)}」</p><div class="folder-select-list"><div class="folder-select-item ${!prompt.folderId ? 'active' : ''}" data-folder-id=""><span>📋 不在文件夹中</span></div>${folders.map(f => `<div class="folder-select-item ${prompt.folderId === f.id ? 'active' : ''}" data-folder-id="${f.id}"><span>📁 ${escapeHtml(f.name)}</span><span class="folder-select-count">${getFolderPrompts(f.id).length}</span></div>`).join('')}</div></div><div class="tm-modal-footer"><button class="tm-btn tm-btn-secondary tm-move-action" data-action="closeMoveModal">取消</button></div></div></div>`);
      $('#tm-moveToFolderModal').on('click', '.folder-select-item', async function() {
        const folderId = $(this).data('folder-id');
        if (folderId) { prompt.folderId = folderId; }
        else { delete prompt.folderId; }
        await saveData();
        TheaterManager.closeMoveModal();
        TheaterManager.renderPromptList();
      });
    },
    ...
    renderGeneratePage() {
      ...
      container.html(`<div class="gen-page"><div class="gen-toolbar"><input type="text" id="tm-genSearchInput" placeholder="🔍 搜索小剧场..."><select id="tm-genCategoryFilter"><option value="">全部分类</option>${categories.map(c => `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join('')}</select></div><div id="tm-genPromptList" class="gen-prompt-list"></div><div id="tm-genResultArea" class="gen-result-area" style="display:none;"><div class="gen-result-header"><h3>🎬 演绎结果</h3><div class="gen-result-btns"><button class="tm-btn tm-btn-small tm-btn-secondary tm-gen-action" data-action="backToList">← 返回</button></div></div><div id="tm-genResultContent" class="gen-result-content"></div><div id="tm-genResultActions" class="gen-result-actions" style="display:none;"><button class="tm-btn tm-btn-secondary tm-gen-action" data-action="copyResult">📋 复制</button><button class="tm-btn tm-btn-secondary tm-gen-action" data-action="insertResultToInput">📝 插入输入框</button><button class="tm-btn tm-btn-primary tm-gen-action" data-action="regenerate">🔄 重新演绎</button></div></div></div>`);
      this.renderGenPromptList();
    },

    renderGenPromptList() {
      const $ = getJQuery();
      const container = $('#tm-genPromptList');
      const prompts = getFilteredPrompts(true);
      if (prompts.length === 0) { container.html(`<div class="tm-empty-small">没有找到小剧场</div>`); return; }
      container.html(prompts.map(p => {
        const folderInfo = p.folderId ? theaterData.folders.find(f => f.id === p.folderId) : null;
        const folderBadge = folderInfo ? `<span class="gen-folder-badge">📁 ${escapeHtml(folderInfo.name)}</span>` : '';
        return `<div class="gen-prompt-card ${p.favorite ? 'favorite' : ''}" data-id="${p.id}"><div class="gen-card-info"><div class="gen-card-title">${p.favorite ? '⭐ ' : ''}${escapeHtml(p.title)} ${folderBadge}</div><div class="gen-card-categories">${renderCategoryTags(p.category)}</div><div class="gen-card-preview">${escapeHtml(p.content?.substring(0, 60) || '')}${(p.content?.length || 0) > 60 ? '...' : ''}</div></div><div class="gen-card-btns"><button class="tm-btn tm-btn-small tm-btn-secondary tm-gen-action" data-action="previewBeforeGen" data-id="${p.id}">👁️</button><button class="tm-btn tm-btn-small tm-btn-primary tm-gen-action" data-action="startGenerate" data-id="${p.id}">🎬 演绎</button></div></div>`;
      }).join(''));
    },
    ...
    showGenMacroModal(id) {
      const $ = getJQuery();
      const prompt = theaterData.prompts.find(p => p.id === id);
      if (!prompt) return;
      $('#tm-genMacroModal').remove();
      $('body').append(`<div id="tm-genMacroModal" class="tm-modal-overlay"><div class="tm-modal" style="max-width:500px;"><div class="tm-modal-header"><h2>🎬 后台演绎</h2><button class="tm-modal-close tm-gen-macro-action" data-action="closeGenMacroModal">×</button></div><div class="tm-modal-body"><p style="margin-bottom:12px;font-weight:600;color:var(--tm-text);">${escapeHtml(prompt.title)}</p><div class="form-group"><label>角色名 →</label><input type="text" id="tm-genMacroChar" class="form-input" value="${escapeHtml(currentCharacter || '角色')}"></div><div class="form-group"><label>用户名 →</label><input type="text" id="tm-genMacroUser" class="form-input" value="${escapeHtml(currentUser || '用户')}"></div><div class="form-group"><label>预览</label><div id="tm-genMacroPreview" class="preview-box preview-small">${escapeHtml(replaceMacros(prompt.content))}</div></div></div><div class="tm-modal-footer"><button class="tm-btn tm-btn-secondary tm-gen-macro-action" data-action="closeGenMacroModal">取消</button><button class="tm-btn tm-btn-primary tm-gen-macro-action" data-action="confirmGenerate" data-id="${id}">🎬 开始演绎</button></div></div></div>`);
      const updatePreview = () => { $('#tm-genMacroPreview').text(replaceMacros(prompt.content, $('#tm-genMacroChar').val(), $('#tm-genMacroUser').val())); };
      $('#tm-genMacroChar, #tm-genMacroUser').on('input', updatePreview);
    },

    closeGenMacroModal() { getJQuery()('#tm-genMacroModal').remove(); },
    startGenerate(id) { this.showGenMacroModal(id); },

    async confirmGenerate(id) {
      const $ = getJQuery();
      const prompt = theaterData.prompts.find(p => p.id === id);
      if (!prompt) return;
      const charVal = $('#tm-genMacroChar').val();
      const userVal = $('#tm-genMacroUser').val();
      const processedContent = replaceMacros(prompt.content, charVal, userVal);
      this.closeGenMacroModal();
      currentGenerationPromptId = id;
      $('#tm-genPromptList').hide();
      $('.gen-toolbar').hide();
      $('#tm-genResultArea').show();
      $('#tm-genResultActions').hide();
      $('#tm-genResultContent').html(`<div class="gen-loading"><div class="gen-loading-spinner"></div><p>正在演绎中...</p><p class="gen-loading-tip">「${escapeHtml(prompt.title)}」</p></div>`);
      isGenerating = true;
      try {
        const result = await triggerSlashWithResult(`/gen lock=on ${processedContent}`);
        isGenerating = false;
        if (result) {
          theaterData.generationHistory = theaterData.generationHistory || [];
          theaterData.generationHistory.unshift({ promptId: id, promptTitle: prompt.title, input: processedContent, output: result, time: Date.now() });
          theaterData.generationHistory = theaterData.generationHistory.slice(0, 50);
          await saveData();
          this.recordUsage(id);
          $('#tm-genResultContent').html(`<div class="gen-success"><div class="gen-success-header"><span class="gen-success-icon">✨</span><span>演绎完成</span></div><div class="gen-output-box">${escapeHtml(result)}</div></div>`);
          $('#tm-genResultActions').show();
          getParentWindow()._tmLastGenResult = result;
        } else {
          $('#tm-genResultContent').html(`<div class="gen-error"><span class="gen-error-icon">⚠️</span><p>演绎失败或返回为空</p></div>`);
        }
      } catch (e) {
        isGenerating = false;
        $('#tm-genResultContent').html(`<div class="gen-error"><span class="gen-error-icon">❌</span><p>演绎出错</p></div>`);
      }
    },

    backToList() {
      const $ = getJQuery();
      $('#tm-genResultArea').hide();
      $('#tm-genPromptList').show();
      $('.gen-toolbar').show();
      currentGenerationPromptId = null;
    },

    async copyResult() {
      const result = getParentWindow()._tmLastGenResult;
      if (result) { await copyToClipboard(result); alert('✅ 已复制'); }
    },

    async insertResultToInput() {
      const result = getParentWindow()._tmLastGenResult;
      if (result) { await triggerSlash(`/setinput ${result}`); this.closeModal(); }
    },
    ...
    renderStats() {
      const $ = getJQuery();
      const container = $('#tm-promptList');
      const prompts = theaterData.prompts;
      const totalPrompts = prompts.length;
      const totalUses = prompts.reduce((sum, p) => sum + (p.useCount || 0), 0);
      const favorites = prompts.filter(p => p.favorite).length;
      const folders = theaterData.folders?.length || 0;
      const categoryStats = {};
      prompts.forEach(p => {
        const cats = normalizeCategories(p.category);
        if (cats.length === 0) {
          if (!categoryStats['未分类']) categoryStats['未分类'] = { count: 0, uses: 0 };
          categoryStats['未分类'].count++;
          categoryStats['未分类'].uses += p.useCount || 0;
        } else {
          cats.forEach(cat => {
            if (!categoryStats[cat]) categoryStats[cat] = { count: 0, uses: 0 };
            categoryStats[cat].count++;
            categoryStats[cat].uses += p.useCount || 0;
          });
        }
      });
      const sortedCategories = Object.entries(categoryStats).sort((a, b) => b[1].uses - a[1].uses).slice(0, 8);
      const maxUses = Math.max(...sortedCategories.map(c => c[1].uses), 1);
      const topUsed = [...prompts].sort((a, b) => (b.useCount || 0) - (a.useCount || 0)).slice(0, 5);
      container.html(`<div class="stats-grid"><div class="stat-card"><div class="stat-value">${totalPrompts}</div><div class="stat-label">总数量</div></div><div class="stat-card"><div class="stat-value">${totalUses}</div><div class="stat-label">总使用</div></div><div class="stat-card"><div class="stat-value">${favorites}</div><div class="stat-label">收藏</div></div><div class="stat-card"><div class="stat-value">${folders}</div><div class="stat-label">文件夹</div></div></div><div class="stats-section"><h3>📊 分类分布</h3><div class="category-bars">${sortedCategories.map(([cat, data]) => `<div class="category-bar-item"><div class="category-bar-label">${escapeHtml(cat)}</div><div class="category-bar-track"><div class="category-bar-fill" style="width:${(data.uses/maxUses*100)}%"></div></div><div class="category-bar-value">${data.uses}次</div></div>`).join('')}</div></div><div class="stats-section"><h3>🏆 最常使用</h3>${topUsed.length > 0 ? topUsed.map((p, i) => `<div class="top-item"><div class="top-rank">${['🥇','🥈','🥉','4','5'][i]}</div><div class="top-info"><div class="top-title">${escapeHtml(p.title)}</div><div class="top-stats">${p.useCount || 0}次</div></div><button class="card-btn" data-action="showPreview" data-id="${p.id}">👁️</button></div>`).join('') : '<div class="tm-empty-small">还没有使用记录</div>'}</div>`);
    },

    // ★★★ v4.3 更新：设置页面新增两个排序开关 ★★★
    renderSettings() {
      const $ = getJQuery();
      const container = $('#tm-promptList');
      const currentTheme = theaterData.theme || 'dark';
      const favoritesPageSort = theaterData.settings?.favoritesPageSort !== false;
      const allPageSort = theaterData.settings?.allPageSort === true;

      container.html(`<div class="settings-page">
        <div class="settings-section">
          <h3>⚙️ 排序设置</h3>
          <div class="settings-toggle">
            <label class="toggle-label">
              <input type="checkbox" id="tm-favoritesPageSort" ${favoritesPageSort ? 'checked' : ''}>
              <span class="toggle-text">收藏页智能排序</span>
            </label>
            <p class="toggle-desc">开启：收藏置顶 + 按使用频率排序<br>关闭：仅按拖动顺序排列</p>
          </div>
          <div class="settings-toggle" style="margin-top:12px;">
            <label class="toggle-label">
              <input type="checkbox" id="tm-allPageSort" ${allPageSort ? 'checked' : ''}>
              <span class="toggle-text">全部页智能排序</span>
            </label>
            <p class="toggle-desc">开启：按使用频率排序<br>关闭：仅按拖动顺序排列（默认）</p>
          </div>
        </div>
        <div class="settings-section">
          <h3>🎨 主题切换</h3>
          <div class="theme-grid">${Object.entries(themes).map(([key, theme]) => `<div class="theme-card ${currentTheme === key ? 'active' : ''}" data-theme="${key}"><div class="theme-preview" style="background:${theme.bg}"><div class="theme-preview-header" style="background:${theme.header}"></div></div><div class="theme-name">${theme.name}</div></div>`).join('')}</div>
        </div>
        <div class="settings-section">
          <h3>💾 数据管理</h3>
          <div class="settings-btns">
            <button class="tm-btn tm-btn-primary tm-setting-action" data-action="exportToJSON">📤 导出</button>
            <button class="tm-btn tm-btn-secondary tm-setting-action" data-action="importFromJSON">📥 恢复</button>
          </div>
        </div>
        <div class="settings-section">
          <h3>ℹ️ 关于</h3>
          <div class="about-info">
            <p>🎭 小剧场管理器 v4.3</p>
            <p class="about-muted">独立排序设置 | 文件夹搜索 | 多分类</p>
          </div>
        </div>
      </div>`);

      // ★ 绑定新设置的事件
      $('#tm-favoritesPageSort').on('change', async function() {
        theaterData.settings.favoritesPageSort = $(this).is(':checked');
        await saveData();
      });
      $('#tm-allPageSort').on('change', async function() {
        theaterData.settings.allPageSort = $(this).is(':checked');
        await saveData();
      });
    },
    ...
    updateCategoryFilter() {
      const $ = getJQuery();
      const categories = getAllCategories();
      $('#tm-categoryFilter').html('<option value="">全部分类</option>' + categories.map(c => `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join(''));
    },
    ...
    randomPick() {
      const prompts = getFilteredPrompts();
      if (prompts.length === 0) { alert('没有可选的小剧场~'); return; }
      this.showPreview(prompts[Math.floor(Math.random() * prompts.length)].id);
    },
    ...
    toggleBatchMode() {
      ...
      selectedIds.clear();
      $('#tm-batchModeBtn').toggleClass('active', batchSelectMode);
      this.renderPromptList();
    },
    ...
    async applyBatchCategory() {
      const $ = getJQuery();
      const categoryInput = $('#tm-batchCategoryInput').val().trim();
      const addMode = $('#tm-batchAddMode').is(':checked');
      if (!categoryInput) { alert('请输入分类'); return; }
      const newCats = parseCategories(categoryInput);
      const count = selectedIds.size;
      selectedIds.forEach(id => {
        const prompt = theaterData.prompts.find(p => p.id === id);
        if (prompt) {
          if (addMode) {
            const existing = normalizeCategories(prompt.category);
            prompt.category = [...new Set([...existing, ...newCats])];
          } else {
            prompt.category = newCats;
          }
        }
      });
      await saveData();
    ...
    async batchDelete() {
      ...
      if (!confirm(`确定删除 ${selectedIds.size} 个？`)) return;
      theaterData.prompts = theaterData.prompts.filter(p => !selectedIds.has(p.id));
      await saveData();
      batchSelectMode = false;
      selectedIds.clear();
      this.updateCategoryFilter();
    ...
    showMacroEditor(id, action) {
      ...
      const prompt = theaterData.prompts.find(p => p.id === id);
      if (!prompt) return;
      $('#tm-macroModal').remove();
      $('body').append(`<div id="tm-macroModal" class="tm-modal-overlay"><div class="tm-modal" style="max-width:500px;"><div class="tm-modal-header"><h2>✏️ 宏替换</h2><button class="tm-modal-close tm-macro-action" data-action="closeMacroEditor">×</button></div><div class="tm-modal-body"><div class="form-group"><label>角色名 →</label><input type="text" id="tm-macroChar" class="form-input" value="${escapeHtml(currentCharacter || '角色')}"></div><div class="form-group"><label>用户名 →</label><input type="text" id="tm-macroUser" class="form-input" value="${escapeHtml(currentUser || '用户')}"></div><div class="form-group"><label>预览</label><div id="tm-macroPreview" class="preview-box preview-small">${escapeHtml(replaceMacros(prompt.content))}</div></div></div><div class="tm-modal-footer"><button class="tm-btn tm-btn-secondary tm-macro-action" data-action="closeMacroEditor">取消</button><button class="tm-btn tm-btn-primary tm-macro-action" data-action="${action === 'insert' ? 'confirmMacroInsert' : 'confirmMacroSend'}" data-id="${id}">${action === 'insert' ? '📋 插入' : '📤 发送'}</button></div></div></div>`);
      const updatePreview = () => { $('#tm-macroPreview').text(replaceMacros(prompt.content, $('#tm-macroChar').val(), $('#tm-macroUser').val())); };
      $('#tm-macroChar, #tm-macroUser').on('input', updatePreview);
    },

    closeMacroEditor() { getJQuery()('#tm-macroModal').remove(); },

    async confirmMacroInsert(id) {
      const $ = getJQuery();
      const prompt = theaterData.prompts.find(p => p.id === id);
      if (!prompt) return;
      const content = replaceMacros(prompt.content, $('#tm-macroChar').val(), $('#tm-macroUser').val());
      await triggerSlash(`/setinput ${content}`);
      this.recordUsage(id);
      this.closeMacroEditor();
      this.closeModal();
    },

    async confirmMacroSend(id) {
      const $ = getJQuery();
      const prompt = theaterData.prompts.find(p => p.id === id);
      if (!prompt) return;
      const content = replaceMacros(prompt.content, $('#tm-macroChar').val(), $('#tm-macroUser').val());
    ...
    openAddModal() {
      ...
      $('#tm-modalTitle').text('➕ 添加');
      $('#tm-promptTitle').val('');
      $('#tm-promptCategory').val('');
      $('#tm-promptContent').val('');
      this.updateFavoriteBtn();
      $('#tm-addModal').removeClass('hidden');
    },
    ...
    openEditModal(id) {
      ...
      if (!prompt) return;
      currentEditId = id;
      currentFavorite = prompt.favorite || false;
      $('#tm-modalTitle').text('✏️ 编辑');
      $('#tm-promptTitle').val(prompt.title);
      $('#tm-promptCategory').val(categoriesToString(prompt.category));
      $('#tm-promptContent').val(prompt.content);
      this.updateFavoriteBtn();
      $('#tm-addModal').removeClass('hidden');
    },
    ...
    updateFavoriteBtn() {
      const $ = getJQuery();
      $('#tm-favoriteBtn').toggleClass('active', currentFavorite).text(currentFavorite ? '⭐' : '☆');
    },
    ...
    async savePrompt() {
      const $ = getJQuery();
      const title = $('#tm-promptTitle').val().trim();
      const content = $('#tm-promptContent').val().trim();
      if (!title || !content) { alert('请填写标题和内容！'); return; }
      const categoryInput = $('#tm-promptCategory').val().trim();
      const categories = parseCategories(categoryInput);
      if (currentEditId) {
        const index = theaterData.prompts.findIndex(p => p.id === currentEditId);
        if (index !== -1) theaterData.prompts[index] = { ...theaterData.prompts[index], title, content, category: categories, favorite: currentFavorite, updatedAt: Date.now() };
      } else {
        theaterData.prompts.push({ id: generateId(), title, content, category: categories, favorite: currentFavorite, useCount: 0, createdAt: Date.now(), updatedAt: Date.now() });
      }
      await saveData();
      this.closeAddModal();
      this.updateCategoryFilter();
    ...
    async deletePrompt(id) {
      const prompt = theaterData.prompts.find(p => p.id === id);
      if (!prompt) return;
      deletedItem = { type: 'prompt', data: { ...prompt } };
      theaterData.prompts = theaterData.prompts.filter(p => p.id !== id);
      await saveData();
      this.updateCategoryFilter();
      this.renderPromptList();
      showUndoBar(prompt);
    ...
    async toggleFavorite(id) {
      const prompt = theaterData.prompts.find(p => p.id === id);
      if (prompt) { prompt.favorite = !prompt.favorite; await saveData(); this.renderPromptList(); }
    },
    ...
    async recordUsage(id) {
      const prompt = theaterData.prompts.find(p => p.id === id);
      if (prompt) { prompt.useCount = (prompt.useCount || 0) + 1; prompt.lastUsed = Date.now(); }
      theaterData.history = theaterData.history || [];
      theaterData.history.unshift({ promptId: id, time: Date.now() });
      theaterData.history = theaterData.history.slice(0, 100);
      await saveData();
    },

    exportToJSON() {
      const dataStr = JSON.stringify(theaterData, null, 2);
      const blob = new Blob([dataStr], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = getParentWindow().document.createElement('a');
      a.href = url;
      a.download = `小剧场备份_${formatDateForFile(Date.now())}.json`;
      a.click();
      ...
      reader.onload = async (e) => {
        try {
          ...
          if (imported.prompts && Array.isArray(imported.prompts)) {
            const existingIds = new Set(theaterData.prompts.map(p => p.id));
            const newPrompts = imported.prompts.filter(p => !existingIds.has(p.id)).map(p => ({ ...p, category: normalizeCategories(p.category) }));
            theaterData.prompts = [...theaterData.prompts, ...newPrompts];
            if (imported.folders) {
              const existingFolderIds = new Set(theaterData.folders.map(f => f.id));
              const newFolders = imported.folders.filter(f => !existingFolderIds.has(f.id));
              theaterData.folders = [...theaterData.folders, ...newFolders];
            }
            await saveData();
            self.updateCategoryFilter();
            self.renderPromptList();
            alert(`✅ 恢复 ${newPrompts.length} 个`);
          } else { alert('❌ 无效格式'); }
    ...
    handleTxtImport(event) {
      ...
      reader.onload = async (e) => {
        ...
        if (blocks.length === 0) { alert('❌ 未找到有效内容'); return; }
        const entriesWithCategory = [];
        const entriesWithoutCategory = [];
        for (const block of blocks) {
          const parsed = parseTheaterEntry(block);
          if (parsed) {
            if (parsed.category && parsed.category.length > 0) entriesWithCategory.push(parsed);
            else entriesWithoutCategory.push(parsed);
          }
        }
        if (entriesWithCategory.length === 0 && entriesWithoutCategory.length === 0) { alert('❌ 无法解析内容'); return; }
        const allEntries = [...entriesWithCategory, ...entriesWithoutCategory];
        const { duplicates, newEntries } = findDuplicates(allEntries);
        getParentWindow()._tmAllNewEntries = newEntries;
        getParentWindow()._tmEntriesWithCategory = entriesWithCategory.filter(e => !duplicates.find(d => d.entry === e));
        getParentWindow()._tmEntriesWithoutCategory = entriesWithoutCategory.filter(e => !duplicates.find(d => d.entry === e));
        if (duplicates.length > 0) {
          self.showDuplicateModal(duplicates, newEntries, entriesWithCategory.filter(e => !duplicates.find(d => d.entry === e)).length);
        } else {
          if (entriesWithoutCategory.length > 0) { self.showCategoryPrompt(entriesWithCategory.length, entriesWithoutCategory); }
          else { self.showTitlePrompt(entriesWithCategory); }
        }
      };
      reader.readAsText(file);
      event.target.value = '';
    ...
    confirmThisTitle() {
      ...
      const entries = getParentWindow()._tmTitleEntries || [];
      const currentIndex = getParentWindow()._tmTitleIndex || 0;
      const newTitle = $('#tm-editTitle').val().trim();
      if (newTitle) entries[currentIndex].title = newTitle;
      getParentWindow()._tmTitleIndex = currentIndex + 1;
      this.showSingleTitleModal();
    },
    ...
    async importEntriesDirectly(entries) {
      for (const entry of entries) {
        const cats = entry.category && entry.category.length > 0 ? entry.category : ['未分类'];
        theaterData.prompts.push({ id: generateId(), title: entry.title, content: entry.content, category: cats, favorite: false, useCount: 0, createdAt: Date.now() });
      }
      await saveData();
      this.updateCategoryFilter();
      this.renderPromptList();
      alert(`✅ 成功导入 ${entries.length} 个！`);
    },

    showDuplicateModal(duplicates, newEntries, alreadyImported) {
      const $ = getJQuery();
      $('#tm-duplicateModal').remove();
      getParentWindow()._tmDuplicates = duplicates;
      getParentWindow()._tmNewEntries = newEntries;
      $('body').append(`<div id="tm-duplicateModal" class="tm-modal-overlay"><div class="tm-modal" style="max-width:520px;"><div class="tm-modal-header"><h2>🔍 检测到重复</h2><button class="tm-modal-close tm-duplicate-action" data-action="closeDuplicateModal">×</button></div><div class="tm-modal-body"><p>发现 <strong style="color:#ff6b6b;">${duplicates.length}</strong> 个重复，${newEntries.length} 个新内容</p></div><div class="tm-modal-footer" style="flex-direction:column;gap:8px;"><button class="tm-btn tm-btn-secondary tm-duplicate-action" data-action="skipDuplicates" style="width:100%;">⏭️ 跳过重复 (${newEntries.length}个)</button><button class="tm-btn tm-btn-primary tm-duplicate-action" data-action="overwriteDuplicates" style="width:100%;">🔄 覆盖 (${duplicates.length + newEntries.length}个)</button><button class="tm-btn tm-btn-secondary tm-duplicate-action" data-action="closeDuplicateModal" style="width:100%;margin-top:4px;">❌ 取消</button></div></div></div>`);
    },
    ...
    async skipDuplicates() {
      const newEntries = getParentWindow()._tmNewEntries || [];
      this.closeDuplicateModal();
      if (newEntries.length === 0) { alert(`✅ 没有新内容`); this.renderPromptList(); return; }
      const needCategory = newEntries.filter(e => !e.category || e.category.length === 0);
      if (needCategory.length > 0) { this.showCategoryPrompt(newEntries.filter(e => e.category && e.category.length > 0).length, needCategory); }
      else { this.showTitlePrompt(newEntries); }
    },

    async overwriteDuplicates() {
      const duplicates = getParentWindow()._tmDuplicates || [];
      const newEntries = getParentWindow()._tmNewEntries || [];
      this.closeDuplicateModal();
      duplicates.forEach(d => {
        const index = theaterData.prompts.findIndex(p => p.id === d.existing.id);
        if (index !== -1) {
          theaterData.prompts[index] = { ...theaterData.prompts[index], title: d.entry.title, content: d.entry.content, category: d.entry.category && d.entry.category.length > 0 ? d.entry.category : theaterData.prompts[index].category, updatedAt: Date.now() };
        }
      });
      await saveData();
      if (newEntries.length > 0) {
        const needCategory = newEntries.filter(e => !e.category || e.category.length === 0);
        if (needCategory.length > 0) { this.showCategoryPrompt(duplicates.length + newEntries.filter(e => e.category && e.category.length > 0).length, needCategory); }
        else { getParentWindow()._tmTitleEntries = newEntries; this.showTitlePrompt(newEntries); }
      } else { this.updateCategoryFilter(); this.renderPromptList(); alert(`✅ 已覆盖 ${duplicates.length} 个`); }
    },

    showCategoryPrompt(alreadyImportedCount, entriesWithoutCategory) {
      const $ = getJQuery();
      $('#tm-categoryPromptModal').remove();
      getParentWindow()._tmPendingEntries = entriesWithoutCategory;
      getParentWindow()._tmAlreadyImported = alreadyImportedCount;
      $('body').append(`<div id="tm-categoryPromptModal" class="tm-modal-overlay"><div class="tm-modal" style="max-width:420px;"><div class="tm-modal-header"><h2>📂 分类设置</h2><button class="tm-modal-close tm-category-action" data-action="closeCategoryPrompt">×</button></div><div class="tm-modal-body">${alreadyImportedCount > 0 ? `<p style="color:#51cf66;">✅ 已处理 ${alreadyImportedCount} 个</p>` : ''}<p>还有 <strong style="color:var(--tm-accent);">${entriesWithoutCategory.length}</strong> 个无分类</p></div><div class="tm-modal-footer" style="flex-direction:column;gap:8px;"><button class="tm-btn tm-btn-secondary tm-category-action" data-action="skipCategoryAll" style="width:100%;">🏷️ 全部「未分类」</button><button class="tm-btn tm-btn-primary tm-category-action" data-action="showUnifiedCategory" style="width:100%;">📁 统一设置</button><button class="tm-btn tm-btn-accent tm-category-action" data-action="startOneByOne" style="width:100%;">✨ 逐个设置</button><button class="tm-btn tm-btn-secondary tm-category-action" data-action="closeCategoryPrompt" style="width:100%;margin-top:4px;">❌ 取消</button></div></div></div>`);
    },
    ...
    async skipCategoryAll() {
      const entries = getParentWindow()._tmPendingEntries || [];
      const entriesWithCategory = getParentWindow()._tmEntriesWithCategory || [];
      this.closeCategoryPrompt();
      entries.forEach(e => e.category = ['未分类']);
      this.showTitlePrompt([...entriesWithCategory, ...entries]);
    },

    showUnifiedCategory() {
      const $ = getJQuery();
      $('#tm-categoryPromptModal .tm-modal-body').html(`<p>统一设置分类：</p><input type="text" id="tm-unifiedCategory" class="form-input" placeholder="分类名称..." autofocus><div class="form-group" style="margin-top:12px;"><label style="display:flex;align-items:center;gap:8px;"><input type="checkbox" id="tm-editTitlesAfter"> 之后逐个修改标题</label></div>`);
      $('#tm-categoryPromptModal .tm-modal-footer').html(`<button class="tm-btn tm-btn-secondary tm-category-action" data-action="closeCategoryPrompt">取消</button><button class="tm-btn tm-btn-primary tm-category-action" data-action="applyUnifiedCategory">确认</button>`);
      $('#tm-unifiedCategory').focus();
    },

    async applyUnifiedCategory() {
      const $ = getJQuery();
      const entries = getParentWindow()._tmPendingEntries || [];
      const entriesWithCategory = getParentWindow()._tmEntriesWithCategory || [];
      const categoryInput = $('#tm-unifiedCategory').val().trim();
      const categories = categoryInput ? parseCategories(categoryInput) : ['未分类'];
      const editTitles = $('#tm-editTitlesAfter').is(':checked');
      entries.forEach(e => e.category = categories);
      const allEntries = [...entriesWithCategory, ...entries];
      this.closeCategoryPrompt();
      if (editTitles) { this.showTitlePrompt(allEntries); }
      else { await this.importEntriesDirectly(allEntries); }
    },
    ...
    showSingleCategoryModal() {
      const $ = getJQuery();
      const entries = getParentWindow()._tmPendingEntries || [];
      const currentIndex = getParentWindow()._tmCurrentIndex || 0;
      if (currentIndex >= entries.length) { this.finishOneByOne(); return; }
      const entry = entries[currentIndex];
      $('#tm-categoryPromptModal').hide();
      $('#tm-singleCategoryModal').remove();
      $('body').append(`<div id="tm-singleCategoryModal" class="tm-modal-overlay"><div class="tm-modal" style="max-width:500px;"><div class="tm-modal-header"><h2>✏️ 设置 (${currentIndex + 1}/${entries.length})</h2><button class="tm-modal-close tm-single-action" data-action="skipRestAndFinish">×</button></div><div class="tm-modal-body"><div class="form-group"><label>标题</label><input type="text" id="tm-singleTitle" class="form-input" value="${escapeHtml(entry.title)}"></div><div class="form-group"><label>分类</label><input type="text" id="tm-singleCategory" class="form-input" placeholder="留空则「未分类」"></div><div class="form-group"><label>预览</label><div class="preview-box preview-small">${escapeHtml(entry.content.substring(0, 150))}</div></div></div><div class="tm-modal-footer"><button class="tm-btn tm-btn-secondary tm-single-action" data-action="skipRestAndFinish">跳过剩余</button><button class="tm-btn tm-btn-secondary tm-single-action" data-action="skipThisOne">跳过</button><button class="tm-btn tm-btn-primary tm-single-action" data-action="confirmThisOne">确认 →</button></div></div></div>`);
      $('#tm-singleTitle').focus();
    },

    async confirmThisOne() {
      const $ = getJQuery();
      const entries = getParentWindow()._tmPendingEntries || [];
      const currentIndex = getParentWindow()._tmCurrentIndex || 0;
      const entry = entries[currentIndex];
      const newTitle = $('#tm-singleTitle').val().trim();
      const categoryInput = $('#tm-singleCategory').val().trim();
      if (newTitle) entry.title = newTitle;
      entry.category = categoryInput ? parseCategories(categoryInput) : ['未分类'];
      theaterData.prompts.push({ id: generateId(), title: entry.title, content: entry.content, category: entry.category, favorite: false, useCount: 0, createdAt: Date.now() });
      await saveData();
      getParentWindow()._tmCurrentIndex = currentIndex + 1;
      this.showSingleCategoryModal();
    },
    ...
    async skipThisOne() {
      ...
      const currentIndex = getParentWindow()._tmCurrentIndex || 0;
      const entry = entries[currentIndex];
      theaterData.prompts.push({ id: generateId(), title: entry.title, content: entry.content, category: ['未分类'], favorite: false, useCount: 0, createdAt: Date.now() });
      await saveData();
      getParentWindow()._tmCurrentIndex = currentIndex + 1;
      this.showSingleCategoryModal();
    },

    async skipRestAndFinish() {
      const entries = getParentWindow()._tmPendingEntries || [];
      const currentIndex = getParentWindow()._tmCurrentIndex || 0;
      for (let i = currentIndex; i < entries.length; i++) {
        theaterData.prompts.push({ id: generateId(), title: entries[i].title, content: entries[i].content, category: ['未分类'], favorite: false, useCount: 0, createdAt: Date.now() });
      }
      await saveData();
      this.finishOneByOne();
    },
    ...
    finishOneByOne() {
      const alreadyImported = getParentWindow()._tmAlreadyImported || 0;
      const entries = getParentWindow()._tmPendingEntries || [];
      const entriesWithCategory = getParentWindow()._tmEntriesWithCategory || [];
      this.closeCategoryPrompt();
      if (entriesWithCategory.length > 0) { this.showTitlePrompt(entriesWithCategory); }
      else { this.updateCategoryFilter(); this.renderPromptList(); alert(`✅ 成功导入 ${alreadyImported + entries.length} 个！`); }
    },
  ...
  function addStyles($) {
    ...
    $('head').append(`<style id="theater-manager-styles">
      ...
      .prompt-card .tm-batch-check:hover{border-color:#667eea;background:rgba(102,126,234,0.3)}
      ...
      .card-title{font-weight:600;font-size:0.92em;color:var(--tm-text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:0 1 auto;min-width:0}
      .card-categories{display:flex;gap:4px;flex-wrap:wrap;flex-shrink:0}
      ...
      .card-folder-badge{padding:2px 6px;background:rgba(255,200,100,0.2);border-radius:4px;font-size:0.65em;color:#ffd43b;flex-shrink:0}
      .gen-folder-badge{padding:1px 5px;background:rgba(255,200,100,0.2);border-radius:3px;font-size:0.7em;color:#ffd43b;margin-left:6px}
      ...
      .gen-toolbar input{flex:1}.gen-toolbar select{max-width:120px}
      ...
      @media(max-width:600px){.tm-main-overlay{padding:12px!important}.tm-main-container{max-height:90vh;border-radius:12px}.tm-header{padding:10px 14px}.tm-header h1{font-size:1em}.tm-hbtn{width:32px;height:32px;font-size:0.9em}.tm-toolbar{padding:8px 14px}.tm-content{padding:10px 14px}.stats-grid{grid-template-columns:repeat(2,1fr)}.theme-grid{grid-template-columns:repeat(2,1fr)}.card-btn{width:26px;height:26px;font-size:0.75em}.tm-modal-overlay{padding:14px!important}.tm-modal{border-radius:12px;max-height:85vh}.tm-modal-body{padding:16px}.tm-modal-footer{padding:12px 16px}.tm-favorite-btn{width:32px;height:32px;font-size:1.1em}.tm-undo-bar{padding:10px 14px;border-radius:0 0 12px 12px}.tm-drag-handle{padding:3px 5px;font-size:0.85em}.prompt-card .tm-batch-check{width:20px;height:20px}}
      @media(max-width:400px){.tm-main-overlay{padding:8px!important}.tm-main-container{max-height:92vh}.tm-header{padding:8px 12px}.tm-header h1{font-size:0.95em}.tm-hbtn{width:30px;height:30px;font-size:0.85em}.tm-toolbar{padding:6px 12px;gap:6px}.tm-toolbar input,.tm-toolbar select{padding:6px 10px;font-size:0.8em}.tm-tabs{overflow-x:auto}.tm-tab{padding:10px 4px;font-size:0.72em}.card-btns{gap:2px}.card-btn{width:24px;height:24px;font-size:0.72em}.settings-btns{flex-direction:column}.settings-btns .tm-btn{min-width:100%}.prompt-card .tm-batch-check{width:18px;height:18px}}
    </style>`);
  }
  ...
  function initIntegration() {
    ...
    $('#theater-manager-menu-item').on('click', function(e) {
      e.preventDefault();
      e.stopPropagation();
      $('#extensionsMenu').fadeOut(200);
      createTheaterManagerUI();
    });
    console.log('✅ 小剧场管理器 v4.3 已加载');
  }

  function start() {
    const $ = getJQuery();
    if ($ && $('#extensionsMenu').length) initIntegration();
